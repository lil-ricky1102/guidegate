...
class Trainer(object):
    ...
    def train(self,split_settings):
        ...
        for i in range(self.epochs):
            ...
            print('Validation at Epoch ' + str(self.current_epoch) + ' with validation loss ' + str(val_loss), " AUROC " + str(auroc) + " AUPRC " + str(auprc))
        auroc, auprc, f1, precision, recall, accuracy, mcc, test_loss, thred_optim = self.test(dataloader="test")
        test_lst = ["epoch " + str(self.best_epoch)] + list(map(float2str, [auroc, auprc, f1, precision, recall, accuracy, mcc, thred_optim, test_loss]))
        ...
        self.save_result(split_settings)

        return self.test_metrics

    def save_result(self,exp_type):
        exp_dir = os.path.join(self.output_dir, exp_type)
        os.makedirs(exp_dir, exist_ok=True)

        if self.config["RESULT"]["SAVE_MODEL"]:
            torch.save(self.best_model.state_dict(),
                       os.path.join(exp_dir, f"best_model_epoch_{self.best_epoch}.pth"))
            torch.save(self.model.state_dict(), os.path.join(exp_dir, f"model_epoch_{self.current_epoch}.pth"))
        state = {
        ...
        torch.save(state, os.path.join(exp_dir, f"result_metrics.pt"))

        val_prettytable_file = os.path.join(exp_dir, "valid_markdowntable.txt")
        test_prettytable_file = os.path.join(exp_dir, "test_markdowntable.txt")
        train_prettytable_file = os.path.join(exp_dir, "train_markdowntable.txt")
        df_tps_file = os.path.join(exp_dir, "true_pred_score.csv")
    ...
    def test(self, dataloader="test"):
        ...
        with torch.no_grad():
            self.model.eval()
            for i, (v_s, v_d, v_p, labels) in enumerate(tqdm(data_loader)):
                self.step += 1
                v_s, v_d, v_p, labels = v_s.to(self.device), v_d.to(self.device), v_p.to(self.device), labels.float().to(self.device)
                if dataloader == "val":
                    v_d, v_s, v_p, f, score= self.model(v_s, v_d, v_p)
                elif dataloader == "test":
                    v_d, v_s, v_p, f, score = self.best_model(v_s, v_d, v_p, mode="eval")
                if self.n_class == 1:
                    n, loss_dt = binary_cross_entropy(score, labels)
                    loss_ds = mean_square_error(v_d, v_s)
                else:
